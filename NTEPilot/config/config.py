...
class Config:
    ...
    def update(self, values: dict[str, Any], save: bool = True) -> "Config":
        ...
        allowed = set(paths) | set(aliases)
        unknown = sorted(key for key in values if key not in allowed and key != "name")
        if unknown:
        ...
        for key, value in values.items():
            ...
            self.set_value(path, self.normalize_value(path, value))
    ...
    def load(cls, name: str = DEFAULT_INSTANCE_NAME) -> "Config":
        ...
        data = cls.normalize_keys(json.loads(path.read_text(encoding="utf-8")))
    ...
    def create(cls, name: str = DEFAULT_INSTANCE_NAME, values: dict[str, Any] | None = None) -> "Config":
        normalized = cls.normalize_name(name)
        config = cls(normalized, cls.load_template_data())
        if values:
    ...
    def list_instances(cls) -> list[dict[str, Any]]:
        ...
        for path in sorted(INSTANCES_DIR.glob("*.json")):
            try:
                ...
                data = {}
            data = cls.normalize_keys(data)
            general = data.get("general") if isinstance(data.get("general"), dict) else {}
            name = str(general.get("name") or data.get("name") or path.stem)
            instances.append({"name": name, "path": str(path)})
        return instances

    @classmethod
    def ensure_default_instance(cls) -> None:
        INSTANCES_DIR.mkdir(parents=True, exist_ok=True)
        if any(INSTANCES_DIR.glob("*.json")):
            return
    ...
    def normalize_keys(data: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                normalized[normalized_key] = Config.normalize_keys(value)
            else:
    ...
    def normalize_value(cls, key: str, value: Any) -> Any:
        default = cls.get_from_data(cls.load_template_data(), key)
    ...
    def template_paths(cls) -> dict[str, Any]:
        ...
        walk("", cls.load_template_data())
        return paths

    @classmethod
    def alias_paths(cls) -> dict[str, str]:
        aliases: dict[str, str] = {}
        for path in cls.template_paths():
            leaf = path.rsplit(".", 1)[-1]
            aliases.setdefault(leaf, path)
        return aliases
    ...
    def migrate_flat_data(data: dict[str, Any]) -> dict[str, Any]:
        ...
        general_keys = {"name", "serial", "package_name", "activity_name", "websocket_host", "websocket_port"}
