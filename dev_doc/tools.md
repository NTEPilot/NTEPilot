...
- 启动事件驱动的异步控制线程 `_control_loop()`
- 主线程在 `temporary_screenshot_interval(0)` 内尽可能连续截图，并用截图前后时间的中点作为帧时间戳
- 主线程每帧检测绿条和光标位置，结合配置项中的输入延迟、黄标速度和绿条速度计算一条确定的 `FishControlSignal`
- **迟滞控制**：光标进入安全区但仍在向绿条中心靠拢时，继续保持当前按键方向运动直至到达中心，从而消除安全边界处的高频按键振荡与左右横跳
- **中值滤波**：记录识别到的绿条长度，根据配置 `green_bar_length_tolerance` 过滤并舍弃长度偏离中位数过大的噪点帧
- 输入速度历史通过 `threading.Lock` 同步，用于黄标位置预测

### 控制线程 (`_control_loop`)

- 通过 `threading.Condition` 等待主线程的新控制信号或当前信号到期，不做固定 sleep 的高频空转
- 同向信号直接更新截止时间，避免重复释放和按下
- 反向信号立即释放当前方向并按下新方向，释放信号立即停止当前输入
- 单条按住信号最长限制为 0.25s，避免截图异常或信号丢失时过度按住
...
流程：
1. `goto_movie()` — 传送到传送点 49，通过移动指令到达电影院
